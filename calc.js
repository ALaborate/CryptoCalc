 ...
 * @param {Number} numb 
 */
function GenericEuler(numb) {

    if (GenericEuler.primes == undefined)
        GenericEuler.primes = PoweredPrimeFactors(PrimeFactors(numb));
    var product = 1;
    GenericEuler.primes.forEach((v) => { product *= Math.pow(v[0], v[1]); });

    if (product != numb) {
        GenericEuler.primes = undefined;
        return GenericEuler(numb);
    }


    return LCM(GenericEuler.primes.map((v) => {
...
function Order(numb, mod) {
    if (Order.lastMod == undefined || Order.lastMod != mod) {
        Order.lastMod = mod;
        Order.GE = GenericEuler(mod);
        Order.divisors = Divisors(Order.GE);
    }

    for (var i = 0; i < Order.divisors.length; i++) {
        if (Power(numb, Order.divisors[i], mod) == 1)
            return Order.divisors[i];
    }
}
/**
 * 
